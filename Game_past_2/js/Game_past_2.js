...
// DOM 요소를 참조하기 위해 document.getElementById() 사용
...
async function getScoreFromDB() {
    try {
        ...
        if (data.success) {
            ...
            console.error("Error fetching the score:", data.message);
        }
...
let result = setInterval(() => {
    ...
    if (
        ...
        if (!scoreAdded) {
            playerScore += 1;
            score.innerHTML = `Score <b>${playerScore}</b>`;
            // updateCrystalValue(playerScore); // 크리스탈볼과 캐릭터가 충돌할 때 크리스탈 값을 1로 업데이트
            saveScoreToDB(playerScore);
